...
use jvm_core::{FieldRef, FieldTrait, JavaClassRef, JavaClassTrait, MemberTrait, Modifier};
use util::Result;
pub fn resove_field(environment: &Environment, index: u16) -> Result<FieldRef> {
    let field_symbol = environment
        .constants
        .get(index as usize)
        .ok_or_else(|| format_err!("invalid index of constant pool"))?
        .try_as_field_ref()?;
    let class = environment
        .class_loader
        .get_loaded_class(&field_symbol.class.symbol.name)?;
    let field = class.get_declared_field(field_symbol.name())?;
    let modifiers = field.modifiers();
    if modifiers.is_private()
        && !environment
            .class
            .equal((&*field.declaring().unwrap()) as &dyn JavaClassTrait)?
        || modifiers.is_protected()
            && !environment
                .class
                .is_assignable(&field.declaring().unwrap())?
        || !modifiers.is_public()
    {
        todo!()
    };
