...
use util::Result;
use vm_core::{ClassLoaderRef, ExecutableRef, GeneraicTypeTrait, JavaClassTrait, PrimitiveType, Type};
use FrameDataType::*;
#[derive(Clone, Debug)]
...
impl FrameDataType {
    pub fn is_assignable(&self, other: &Self) -> Result<bool> {
        ...
        Ok(self == other
            || match (self, other) {
                ...
                (UninitializedVariable(Some(c0), x), UninitializedVariable(Some(c1), y)) => x == y && c0.equal(&**c1)?,
    ...
    pub fn from_verification_type_info(environment: &Environment, verification_type_info: &VerificationTypeInfo) -> Result<FrameDataType> {
        Ok(match verification_type_info {
            ...
            VerificationTypeInfo::UninitializedThisVariableInfo => UninitializedThis(environment.class.clone()),
            VerificationTypeInfo::ObjectVariableInfo(c) => Reference(environment.class_loader.get_class(&c.symbol.name)?),
            VerificationTypeInfo::UninitializedVariableInfo(offset) => UninitializedVariable(None, offset),
        })
    }
    ...
    pub fn from_type_symbol(class_loader: &ClassLoaderRef, type_symbol: &TypeSymbol) -> Result<Self> {
...
impl FrameState {
    pub fn is_assignable(&self, other: &FrameState) -> Result<bool> {
        Ok(self.stack.len() == other.stack.len()
            && self.stack.iter().zip(other.stack.iter()).map(|(a, b)| a.is_assignable(b)).try_fold(true, |l, c| c.map(|b| l & b))?
            && self.locals.len() == other.locals.len()
            && self.locals.iter().zip(other.locals.iter()).map(|(a, b)| a.is_assignable(b)).try_fold(true, |l, c| c.map(|b| l & b))?
            && (!self.flag_this_uninit | other.flag_this_uninit))
    }

    pub fn stack_top_is_assignable(&self, data_type: &FrameDataType) -> Result<bool> {
        self.stack.last().map(|t| t.is_assignable(data_type)).unwrap_or(Ok(true))
    }

    fn store_one_word(&mut self, index: u16, data_type: FrameDataType) -> Result<()> {
        self.locals.get_mut(index as usize).ok_or_else(|| format_err!("local variable out of range")).map(|p| *p = data_type)
    }
    ...
    pub fn store(&mut self, index: u16, data_type: FrameDataType) -> Result<()> {
        ...
        self.store_one_word(index, data_type).and_then(|_| if is_two_word { self.store_one_word(index + 1, FrameDataType::Top) } else { Ok(()) })
    }

    fn load_match_one_word(&mut self, index: u16, data_type: &FrameDataType) -> Result<()> {
        self.locals.get(index as usize).ok_or_else(|| format_err!("local variable out of range")).and_then(|e| {
            e.is_assignable(data_type).map(|assignable| assignable.then_some(()).ok_or_else(|| format_err!("local variable is not assignable"))).flatten()
        })
    }
    ...
    pub fn pop_match(&mut self, data_type: &FrameDataType) -> Result<()> {
        self.stack.pop().map(|p| p.is_assignable(data_type)).unwrap_or(Ok(true)).and_then(|a| a.then_some(()).ok_or(format_err!("stack not assignable")))
    }
    ...
    pub fn push_locals(&mut self, environment: &mut Environment<'_>, data_type: FrameDataType) -> Result<()> {
    ...
    pub fn new(this_class: Option<&Arc<dyn JavaClassTrait>>, is_constructor: bool, max_stack: u16, max_locals: u16, method: &ExecutableRef) -> Self {
        Self {
            ...
            locals: {
                ...
                if let Some(this) = this_class {
                    locals.push(if is_constructor { UninitializedThis(this.clone()) } else { Reference(this.clone()) });
                }
    ...
    pub fn from_stack_map_table(stack_map_table: &StackMapTable, environment: &mut Environment<'_>) -> Result<HashMap<u32, Self>> {
        ...
        for e in &stack_map_table.entries {
            ...
            match e {
                ...
                StackMapFrame::SameLocals1StackItemFrame { stack, .. } | StackMapFrame::SameLocals1StackItemFrameExtended { stack, .. } => {
                    frame.stack.clear();
                    push(environment.frame, environment.code.max_stack, FrameDataType::from_verification_type_info(environment, stack)?)?;
                }
                StackMapFrame::ChopFrame { frame_type, .. } => {
                    frame.stack.clear();
                    for _ in 0..251 - frame_type {
                        frame.locals.pop().ok_or_else(|| format_err!("stack is too short"))?;
                    }
                }
                ...
                StackMapFrame::FullFrame { locals, stack, .. } => {
                    frame.stack.clear();
                    for s in stack {
                        push(environment.frame, environment.code.max_stack, FrameDataType::from_verification_type_info(environment, s)?)?;
                    }
                    frame.locals.clear();
                    for l in locals {
                        let data_type = FrameDataType::from_verification_type_info(environment, l)?;
...
pub fn load<'l>(frame: &'l mut FrameState, index: u16) -> Result<&'l FrameDataType> {
    frame.locals.get(index as usize).ok_or_else(|| format_err!("invalid local variable index"))
}

pub fn match_locals(frame: &mut FrameState, index: u16, exception: &FrameDataType) -> Result<()> {
    let actually = frame.locals.get(index as usize).ok_or_else(|| format_err!("invalid index:{}", index))?;
    (actually == exception).then_some(()).ok_or_else(|| format_err!("invalid local variable type,except:{:?},actually:{:?}", exception, actually))
}
pub fn pop(frame: &mut FrameState) -> Result<FrameDataType> {
    frame.stack.pop().ok_or_else(|| format_err!("invalid stack size"))
}
...
pub fn peek<'l>(frame: &'l mut FrameState) -> Result<&'l FrameDataType> {
    frame.stack.last().ok_or_else(|| format_err!("invalid stack size"))
}
