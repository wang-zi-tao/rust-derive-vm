extern crate vm_core as vm_core;
use std::{
    any::TypeId,
    fmt::Debug,
    sync::{Arc, Mutex},
...
use inkwell::{context::Context as LLVMContext, execution_engine::ExecutionEngine as LLVMExecutionEngine, module::Module as LLVMModule};
...
impl<'ctx> RuntimeFilter for LLVMRuntimeWraped<'ctx> {
    ...
    fn consume<'l>(
        ...
        let input: Arc<LLVMMoudleBuilder> = source_input.downcast().map_err(|_| format_err!("downcast failed"))?;
        let mut guard = self.0.lock().map_err(|e| format_err!("lock error,message:\n{:#?}", e))?;
        guard.build_module(&*input)?;
        Ok(SmallVec::new())
    }
}
pub type LLVMMoudleBuilder = Box<dyn Send + Sync + for<'ctx> Fn(&'ctx LLVMContext) -> Fallible<LLVMModule>>;
...
impl<'ctx> LLVMRuntime<'ctx> {
    pub fn new(context: &'ctx LLVMContext) -> Fallible<Self> {
        ...
        Ok(Self { context, _root_module: root_module, execute_engine })
    }

    pub fn build_module(&mut self, builder: impl FnOnce(&'ctx LLVMContext) -> Fallible<LLVMModule>) -> Fallible<()> {
        let module = builder(self.context)?;
        self.execute_engine.add_module(&module).map_err(|_| format_err!("error when add module into the execute_engine"))?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn llvm_runtime() {
        ...
        llvm_runtime
            ...
            .unwrap();
        assert_ne!(0, llvm_runtime.execute_engine.get_function_address("f").unwrap());
    }
}
