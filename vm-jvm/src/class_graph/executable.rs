...
use vm_core::ExecutableLayoutTrait;

use crate::{
    annotations::{AnnotatedElement, Annotations},
    class::JavaClass,
    class_loader::ClassLoader,
    generate_type::{GenericDeclaration, GenericType, TemporaryTypeTreeNode, TypeTreeNode, TypeVariable, TypeVariableSet},
...
impl TypeTreeNode for Executable {
    fn get_type_variable(&self, name: &str) -> Option<vm_core::Type> {
        todo!()
    }
}
...
impl AccessibleObject for Executable {
    fn can_access(&self, object: Option<&dyn vm_core::OOPTrait>) -> bool {
        todo!()
    }
...
impl Executable {
    ...
    pub fn new(
        ...
        let symbol = MethodTypeSymbol::new(&method.descriptor).ok_or(format_err!("illegal executable descriptor"))?;
        let (parameters_type_symbol, return_type_type_symbol) = (symbol.parameters, symbol.return_type);
        ...
        let mut throws: Vec<GenericType> = if let Some(Attribute::Exception(e)) = method.attributes.get("Exceptions") {
        ...
        let mut return_type: Option<GenericType> = if &*return_type_type_symbol.name == "V" {
            ...
            Some(GenericType::from_symbol(class_loader, &return_type_type_symbol.name)?)
        };
        let declaring_weak = Arc::downgrade(&declaring);
        let type_variables = if let Some(Attribute::Signature(signature)) = method.attributes.get("Signature") {
            let executable_signature = MethodSymbol::from_attribute(signature)?;
            let temporary_type_variables = TemporaryTypeTreeNode::new(executable_signature.type_parameters, Some(&*declaring), class_loader)?;
            let context = SignatureContext::new(&**class_loader, &temporary_type_variables);
            for (parameter, parameter_signature) in parameters.iter_mut().zip(executable_signature.parameters.iter()) {
                parameter.parameter_type.set_type(parameter_signature.to_type(&context)?);
            }
            for (throw_type, throw_signature) in throws.iter_mut().zip(executable_signature.throws.iter()) {
        ...
        let extends = if let Some(super_class_some) = super_class {
            super_class_some.get_executable(&name_and_parameters)?.cloned()
        } else {
            None
        };
        Ok(Self {
...
impl Parameter {
    ...
    pub fn with_type(parameter_type: GenericType) -> Self {
        Self { parameter_type, annotations: Annotations::default(), modifiers: 0, name: None }
    }
}
impl From<GenericType> for Parameter {
    fn from(parameter_type: GenericType) -> Self {
        Self { parameter_type, annotations: default(), modifiers: default(), name: default() }
    }
}
impl Debug for Executable {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("Executable")
            .field(&format!("{:?} {}.{}{}", &self.return_type, self.declaring().unwrap().binary_name(), self.name, self.descriptor))
            .finish()
    }
}
impl Executable {
