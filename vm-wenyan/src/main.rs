...
#[lexical([],[
"有","名之","以","其","所餘幾何","昔之","者","今","是矣","曰","恆","之","为是","吾有","其物如是","物之","是謂","之物也","矣","批曰","也","注曰","是術曰","欲行是術","必先得","之術也","疏曰",  "數","言","爻","列","物","術",  "陽","陰",  "若","若非","乃止","中有陽乎","中無陰乎","乃歸空無","乃得","為是","遍","恆為是","凡","中之","云云",  "大於","不大於","小於","不小於","等於","不等於","加","减","乘","除","減","夫","銜","長","其餘","書之",
])]
        ...
        lalr1_analyser! {
            解析文言语法:文言词法->(){
                ...
                语句=>()->{
                    ...
                    | [表达式(甲),書之,句号]=>代码.书之(甲);
                    | [若,判断(甲),句号,分块(乙),块(丙),也,句号]=>代码.分支(甲,乙,丙,None);
                    | [若,判断(甲),句号,分块(乙),块(丙),若非,句号,块(丁),也,句号]=>代码.分支(甲,乙,丙,Some(丁));
                    | [循环头(h),循环块尾(丁),云云,句号]=>代码.循环(h,丁);
                ...
                表达式列表=>表达式列表->{
                    [夫,操作数(甲),句号]=>Ok(vec![甲]);
                    | [表达式(甲)]=>Ok(vec![甲]);
                    | [表达式列表(mut l),夫,操作数(甲),句号]=>Ok({l.push(甲);l});
                },
                表达式=>表达式->{
                    [有,类型(_),常量(甲),句号]=>Ok(甲);
                    | [吾有,数(甲),类型(乙),句号,常量列表(c)]=>Ok(c[0].clone());
                ...
                变量名列表=>变量名列表->{ [曰,变量(甲)]=>Ok(vec![甲]); |[变量名列表(mut l),曰,变量(甲)]=>Ok({l.push(甲);l}); },
                常量列表=>表达式列表->{ [曰,常量(甲),句号]=>Ok(vec![甲]); |[常量列表(mut l),曰,常量(甲),句号]=>Ok({l.push(甲);l}); },
            }
        }
        解析文言语法(源代码)?;
        代码.打包()
...
fn main() -> Fallible<()> {
    ...
    let vm = 创建虚拟机()?;
    println!("問天地好在。『wangzi 文言 虚拟机 {} 』", &env!("CARGO_PKG_VERSION"));
    loop {
        print!(" ");
